...
contract MockERC20Token is ERC20 {
    ...
    constructor(
        ...
        require(_mintAddresses.length == _mintAmounts.length, "MockERC20Token: must have same number of mint addresses and amounts");

        for (uint i; i < _mintAddresses.length; i++) {
            require(_mintAddresses[i] != address(0), "MockERC20Token: cannot have a non-address as reserve.");
